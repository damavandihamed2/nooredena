...
portfolio_trade_value = get_trades_value(trade_type=2, start_date=portfolio_start_date, end_date=portfolio_end_date)
portfolio_trade_value.rename({"value": "portfolio_sell_value"}, axis=1, inplace=True)

dataframe = market_trade_value.merge(portfolio_trade_value, on="date", how="outer")
dataframe.name = "daily"
...
dataframe["portfolio_sell_value"] /= 1e9
...
dataframe_weekly__ = dataframe_weekly.groupby(by=["jyear", "JWeekNum"], as_index=False).sum(numeric_only=True)[["jyear", "JWeekNum", "market_trade_value", "portfolio_sell_value"]]
dataframe_weekly = dataframe_weekly_.merge(dataframe_weekly__, on=["jyear", "JWeekNum"], how="left")
dataframe_weekly = dataframe_weekly[["date", "market_trade_value", "portfolio_sell_value"]]


dataframe_monthly = dataframe.copy()
dataframe_monthly.name = "monthly"
dataframe_monthly["year-month"] = dataframe_monthly["date"].str[:7]
dataframe_monthly = dataframe_monthly[["year-month", "market_trade_value", "portfolio_sell_value"]].groupby(
    by="year-month", as_index=False).sum().rename({"year-month": "date"}, axis=1, inplace=False)

dataframe["ratio"] = dataframe["portfolio_sell_value"] / dataframe["market_trade_value"]
dataframe_weekly["ratio"] = dataframe_weekly["portfolio_sell_value"] / dataframe_weekly["market_trade_value"]
dataframe_monthly["ratio"] = dataframe_monthly["portfolio_sell_value"] / dataframe_monthly["market_trade_value"]

dfs_ = [{"name": "daily", "df": dataframe},
        {"name": "weekly", "df": dataframe_weekly},
        {"name": "monthly", "df": dataframe_monthly}]

for d in dfs_:
    name_ = d["name"]
    dataframe_chart = d["df"]
    fig = make_subplots(rows=1, cols=1, specs = [[{"type": "xy", "secondary_y": True}]])
    fig_market = go.Scatter(x=dataframe_chart["date"], y=dataframe_chart["market_trade_value"], name="ارزش معاملات خرد (میلیارد ریال)")
    fig_market.update(line=dict(color=color2, smoothing=1.3, shape="spline"), mode="lines")
    fig.add_trace(fig_market, row=1, col=1)

    fig_portfolio = go.Scatter(x=dataframe_chart["date"], y=dataframe_chart["portfolio_sell_value"], name="فروش (میلیارد ریال)")
    ...
    fig.write_html(f"./annual_meeting/sell_value_{name_}.html", config={"displayModeBar": False})



    fig_ratio = go.Scatter(x=dataframe_chart["date"], y=dataframe_chart["ratio"], name="نسبت فروش پرتفوی به معاملات خرد")
    ...
    fig_.write_html(f"./annual_meeting/sell_ratio_{name_}.html", config={"displayModeBar": False})
