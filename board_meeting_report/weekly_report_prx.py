...
portfolio = weekly_report_data.get_portfolio(include_avalhami=False, main_portfolio=False, prx_portfolio=True)

transfered_symbols = [
    ["خودرو", 877159449156, 1560781938],
    ["آسيا", 196648000000, 94000000],
    ["بمولد", 132228992490, 23076613],
    ["دقاضي", 178370670000, 23782756],
    ["دقاضيح", 8158622780, 1278781],
    ["زاگرس", 105170601550, 753641],
    ["زقيام", 151629139572, 30156949],
    ["زگلدشت", 159067354377, 31667799],
    ["زگلدشتح", 58591416726, 16261842],
    ["سخاش", 126824770590, 13593223],
    ["سفانو", 111606289580, 3035254],
    ["سهگمت", 179607762180, 1439857],
    ["فصبا"	, 123991306824, 42696731],
    ["كساوه", 120455216700, 8824558]
]

for i in range(len(transfered_symbols)):
    if transfered_symbols[i][0] in portfolio["symbol"].values:
        cost_per_share = transfered_symbols[i][1] / transfered_symbols[i][2]
        idx_ = portfolio.index[portfolio["symbol"] == transfered_symbols[i][0]].values[0]
        portfolio["total_cost"].iloc[idx_] = round(portfolio["amount"].iloc[idx_] * cost_per_share)
        portfolio["cost_per_share"].iloc[idx_] = portfolio["total_cost"].iloc[idx_] / portfolio["amount"].iloc[idx_]
        portfolio["profitloss"].iloc[idx_] = portfolio["value"].iloc[idx_] - portfolio["total_cost"].iloc[idx_]
        portfolio["profitloss_percent"].iloc[idx_] = portfolio["profitloss"].iloc[idx_] / portfolio["total_cost"].iloc[idx_]
...
figure_1_0 = go.Table(header=dict(values=["<b>درصد سود (زیان)<b>", "<b>سود (زیان) - میلیارد ریال<b>",
                    ...
                    cells=dict(
                        values=[
                            ...
                            list(range(1, 1 + len(portfolio) - 1)) + [""] * 1
                        ],
...
figure_1_1 = go.Bar(name="ارزش روز", x="<b>" + top_10_value["symbol"] + "<b>", y=top_10_value["value"],
...
figure_1_2 = go.Bar(name="بهای تمام شده کل", x="<b>" + top_10_value["symbol"] + "<b>", y=top_10_value["total_cost"],
...
figure_1_3 = go.Bar(name="مازاد(کاهش) ارزش", x="<b>" + top_10_value["symbol"] + "<b>", y=top_10_value["profitloss"],
...
fig = make_subplots(
    ...
    specs=[[{"type": "table"}], [{"type": "bar", "t": 0.05}]],
    row_heights=[2.5, 1],
    subplot_titles=[
        "<b>" + f"آخرین وضعیت پرتفوی بورسی در سبد - {report_date.replace('-', '/')}" + "<b>",
        "<b>" + "عملکرد 10 سهم بزرگ پرتفوی بورسی در سبد - " + f"{report_date.replace('-', '/')}" + "<b>"])

fig.add_trace(figure_1_0, row=1, col=1)
fig.add_trace(figure_1_1, row=2, col=1)
fig.add_trace(figure_1_2, row=2, col=1)
fig.add_trace(figure_1_3, row=2, col=1)
...
fig.update_layout(
    template="seaborn", font_family=fnt, height=2400,
    margin=dict(autoexpand=False, l=100, r=100, t=50, b=80),
    legend=dict(x=0.9, y=0.185, font=dict(size=16, weight="bold")))

fig.write_html(f"{folder_address}/({report_date}) گزارش عملکرد/prx/report ({report_date})_1.html",
               config={"displayModeBar": False})

########################################################################################################################

buy = weekly_report_data.get_trades_buy(start_date=buy_sell_start_date, end_date=buy_sell_end_date,
                                             fix_exclude=True, main_portfolio=False, prx_portfolio=True)["buy"]
...
sell = weekly_report_data.get_trades_sell(start_date=buy_sell_start_date, end_date=buy_sell_end_date,
                                               fix_exclude=True, main_portfolio=False, prx_portfolio=True)["sell"]
...
for i in range(len(transfered_symbols)):
    if transfered_symbols[i][0] in sell_table["symbol"].values:
        cost_per_share = transfered_symbols[i][1] / transfered_symbols[i][2]
        idx_ = sell_table.index[sell_table["symbol"] == transfered_symbols[i][0]].values[0]
        sell_table["cost"].iloc[idx_] = round(sell_table["amount"].iloc[idx_] * cost_per_share)
...
fig.write_html(f"{folder_address}/({report_date}) گزارش عملکرد/prx/report ({report_date})_2.html", config={"displayModeBar": False})
########################################################################################################################
...
fig.write_html(f"{folder_address}/({report_date}) گزارش عملکرد/prx/report ({report_date})_3.html",
               config={"displayModeBar": False})

########################################################################################################################
########################################################################################################################

bazdehi = pd.read_excel(f"{folder_address}/data/bazdehi_prx.xlsx")
...
fig.add_trace(figure_7, row=1, col=1)

fig.update_annotations(font=dict(size=25, family=fnt))
fig.update_layout(template="seaborn", font_family=fnt,
                  margin=dict(autoexpand=False, l=100, r=100, t=50, b=50),
                  height=1300)
fig.write_html(f"{folder_address}/({report_date}) گزارش عملکرد/prx/report ({report_date})_4.html",
               config={"displayModeBar": False})
