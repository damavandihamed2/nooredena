...
portfolio = weekly_report_data.get_portfolio(include_avalhami=avalhami, main_portfolio=True, prx_portfolio=True)
...
fig.write_html(f"{folder_address}/({report_date}) گزارش عملکرد/{'with' if avalhami else 'without'}_avalhami"
               f"/report ({report_date})_1.html", config={"displayModeBar": False})
...
figure_2 = go.Table(header=dict(values=["<b>درصد سود (زیان)<b>", "<b>سود (زیان) - میلیارد ریال<b>",
                                         "<b>سهم از پرتفوی<b>", "<b>ارزش روز - میلیارد ریال<b>",
                                         "<b>قیمت پایانی - ریال<b>", "<b>بهای تمام شده - میلیارد ریال<b>",
                                         "<b>بهای هر سهم - ریال<b>", "<b>تعداد<b>", "<b>سبد<b>", "<b>درصد مالکیت<b>",
                                         "<b>نماد<b>", "<b>ردیف<b>"],
...
fig = make_subplots(
    rows=1, cols=1, print_grid=False, vertical_spacing=0.005, specs=[[{"type": "table"}]],
    subplot_titles=["<b>" + "آخرین وضعیت پرتفوی بورسی - " + f"{report_date.replace('-', '/')}" + "<b>"])

fig.add_trace(figure_2, row=1, col=1)
...
fig.update_layout(
    ...
    margin=dict(autoexpand=False, l=100, r=100, t=50, b=80),
    legend=dict(x=0.9, y=0.15, font=dict(size=16, weight="bold")))

fig.write_html(f"{folder_address}/({report_date}) گزارش عملکرد/{'with' if avalhami else 'without'}_avalhami"
               f"/report ({report_date})_2.html", config={"displayModeBar": False})
...
figure_3_1 = go.Bar(name="ارزش روز", x="<b>" + top_10_value["symbol"] + "<b>", y=top_10_value["value"],
...
figure_3_2 = go.Bar(name="بهای تمام شده کل", x="<b>" + top_10_value["symbol"] + "<b>", y=top_10_value["total_cost"],
...
figure_3_3 = go.Bar(name="مازاد(کاهش) ارزش", x="<b>" + top_10_value["symbol"] + "<b>", y=top_10_value["profitloss"],
...
fig = make_subplots(
    rows=1, cols=1, print_grid=False, vertical_spacing=0.005, specs=[[{"type": "bar", "b": -0.005}]],
    subplot_titles=["<b>" + "عملکرد 10 سهم بزرگ پرتفوی - " + f"{report_date.replace('-', '/')}" + "<b>"])

fig.add_trace(figure_3_1, row=1, col=1)
fig.add_trace(figure_3_2, row=1, col=1)
fig.add_trace(figure_3_3, row=1, col=1)

fig.update_annotations(font=dict(size=25, family=fnt))
fig.update_xaxes(tickfont=dict(family=fnt, size=20), showgrid=False)
fig.update_yaxes(showticklabels=False, showgrid=False)
fig.update_layout(
    template="seaborn", font_family=fnt, height=900,
    margin=dict(autoexpand=False, l=100, r=100, t=50, b=80),
    legend=dict(x=0.9, y=0.95, font=dict(size=16, weight="bold")))

fig.write_html(f"{folder_address}/({report_date}) گزارش عملکرد/{'with' if avalhami else 'without'}_avalhami"
               f"/report ({report_date})_3.html", config={"displayModeBar": False})

########################################################################################################################

buy = weekly_report_data.get_trades_buy(start_date=buy_sell_start_date, end_date=buy_sell_end_date,
                                             fix_exclude=True, main_portfolio=True, prx_portfolio=True)["buy"]
...
sell = weekly_report_data.get_trades_sell(start_date=buy_sell_start_date, end_date=buy_sell_end_date,
                                               fix_exclude=True, main_portfolio=True, prx_portfolio=True)["sell"]
...
fig.write_html(f"{folder_address}/({report_date}) گزارش عملکرد/{'with' if avalhami else 'without'}_avalhami"
               f"/report ({report_date})_4.html", config={"displayModeBar": False})
...
fig.add_trace(figure_5_1, row=1, col=1)
fig.add_trace(figure_5_2, row=2, col=1)
...
fig.write_html(f"{folder_address}/({report_date}) گزارش عملکرد/{'with' if avalhami else 'without'}_avalhami"
               f"/report ({report_date})_5.html", config={"displayModeBar": False})

########################################################################################################################
########################################################################################################################

bazdehi = pd.read_excel(f"{folder_address}/data/bazdehi{'_avalhami' if avalhami else ''}.xlsx")
...
fig.write_html(f"{folder_address}/({report_date}) گزارش عملکرد/{'with' if avalhami else 'without'}_avalhami"
               f"/report ({report_date})_6.html", config={"displayModeBar": False})

########################################################################################################################
########################################################################################################################

bazdehi = pd.read_excel(f"{folder_address}/data/bazdehi_fiscal{'_avalhami' if avalhami else ''}.xlsx")
bazdehi = bazdehi[bazdehi["ارقام به میلیارد ریال"] != "پرداخت به همراه اول"].reset_index(drop=True)
...
figure_7 = go.Table(
    header=dict(
        values=[f"<b>{bazdehi_columns[i]}<b>" for i in range(len(bazdehi_columns))], fill=dict(color=color2),
    ...
    cells=dict(
        ...
        [[",.0f"] * 3 + ([",.0f"] + ["(.1%"] + ["(.1%"]) * 4] * (((len(bazdehi_columns) - 5) % 3) > 0) +

        [[",.0f"] * 3 + ([",.0f"] + ["(.1%"] + [""]) * 4] * (((len(bazdehi_columns) - 5) % 3) // 2) +

        ([[",.0f"] * 3 + ([",.0f"] + ["(.1%"] + ["(.1%"]) * 4] +
         [[",.0f"] * 3 + ([",.0f"] + ["(.1%"] + [""]) * 4] * 2) * ((len(bazdehi_columns) - 5) // 3) +
...
fig.update_layout(template="seaborn", font_family=fnt,
                  ...
                  height=1300)
fig.write_html(f"{folder_address}/({report_date}) گزارش عملکرد/{'with' if avalhami else 'without'}_avalhami"
               f"/report ({report_date})_7.html", config={"displayModeBar": False})
