...
last_date = pd.read_sql("SELECT Max(date) FROM [nooredenadb].[brokers].[ime_trades_coinonline]", db_conn)[""].iloc[0]
...
for b in range(len(brokers_list)):
    ...
    agent.login()
    trades_tmp = agent.get_trades(start_date=last_date, end_date=today, page_size=100)
...
if not trades_df.empty:
    crsr = db_conn.cursor()
    crsr.execute(f"DELETE FROM [nooredenadb].[brokers].[ime_trades_coinonline] WHERE date >= '{last_date}'")
