...
class Market:
    ...
    def send_req(self, s_date: str, e_date: str) -> None:
...
market_test = init_markets()[12]
response = market_test.send_req("1404/06/01", "1405/03/05")
...
def get_market_data(
        market_id: int, s_date: str, e_date: str) -> dict[list[dict]]:
...
s_date = '1401/12/4'
e_date = '1402/06/31'
output = get_market_data(markets, market_id, s_date, e_date)



class Markets:
    Physic = Market(11, 'فیزیکی', 'subsystems/ime/services/home/imedata.asmx/GetAmareMoamelatList', 'post', 'Symbol' , fari = False)
    Other = Market(5, 'فرعی', 'subsystems/ime/services/home/imedata.asmx/GetAmareMoamelatList', 'post', 'Symbol', fari = True)

df = []
physic = Markets.Physic()

df.append(physic.data)

def get_market_data(
        markets: list[Markets], s_date: str, e_date: str) -> dict[list[dict]]:

    if s_date > e_date:
        raise ValueError("start_date cannot be greater than end date")
