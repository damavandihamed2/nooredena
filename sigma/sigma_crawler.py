import pandas as pd
import sigma, warnings, jdatetime
...
portfoId = 1
query_last_buysell = f"select max(date) date FROM [nooredenadb].[sigma].[buysell] WHERE portfolio_id={portfoId}"
last_buysell = pd.read_sql(query_last_buysell, powerbi_database)["date"].iloc[0]
...
portfoId = 1
query_last_profitloss = f"select max(date) date FROM [nooredenadb].[sigma].[profitloss] WHERE portfolio_id={portfoId}"
last_profitloss = pd.read_sql(query_last_profitloss, powerbi_database)["date"].iloc[0]
...
portfoId = 1
query_last_portfolio = f"SELECT MAX(date) date FROM [nooredenadb].[sigma].[portfolio] WHERE portfolio_id={portfoId}"
last_portfolio = pd.read_sql(query_last_portfolio, powerbi_database)["date"].iloc[0]
start_portfolio = (jdatetime.datetime.strptime(last_portfolio, format="%Y/%m/%d") + one_day).strftime("%Y/%m/%d")
query_end_portfolio = f"SELECT MAX(date) date FROM [nooredenadb].[sigma].[buysell] WHERE portfolio_id={portfoId}"
end_portfolio = pd.read_sql(query_end_portfolio, powerbi_database)["date"].iloc[0]

if end_portfolio >= start_portfolio:
    portfolio = pd.DataFrame()
    date_ = start_portfolio
    while date_ <= end_portfolio:
        print(f"getting portfolio data for ({date_})")
...
portfoId = 1
query_lst_dividend = f"SELECT MAX(meeting_date) date FROM [nooredenadb].[sigma].[dividend] WHERE portfolio_id={portfoId}"
lst_dividend = pd.read_sql(query_lst_dividend, powerbi_database)["date"].iloc[0]
query_end_dividend = f"SELECT MAX(date) date FROM [nooredenadb].[sigma].[portfolio] WHERE portfolio_id={portfoId}"
end_dividend = pd.read_sql(query_end_dividend, powerbi_database)["date"].iloc[0]
...
if len(dividend_df) > 0:
    ...
    crsr.execute(f"DELETE FROM [nooredenadb].[sigma].[dividend] WHERE meeting_date >= '{lst_dividend}'")
    crsr.close()
